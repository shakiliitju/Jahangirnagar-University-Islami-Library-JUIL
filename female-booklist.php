<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>বইয়ের তালিকা - জাবিইপা</title>
    <link rel="shortcut icon" href="image/juil-logo.ico" type="image/x-icon">

    <link rel="stylesheet" href="https://unpkg.com/swiper@7/swiper-bundle.min.css" />
    <!-- font awesome cdn link  -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">
    <script src="https://kit.fontawesome.com/899adf5c9e.js" crossorigin="anonymous"></script>
    <!-- custom css file link  -->
    <link rel="stylesheet" href="css/style.css">
    <!-- bootstrap link  -->
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css">
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.1/jquery.min.js"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js"></script>

    <meta name="keywords" content="জাবিইপা" />
    <meta name="description" content="জাহাঙ্গীরনগর বিশ্ববিদ্যালয় ইসলামি পাঠাগার" />
    <meta name="twitter:card" content="summary">
    <meta name="twitter:title" content="হোম  -  জাবিইপা">
    <meta name="twitter:description" content="জাহাঙ্গীরনগর বিশ্ববিদ্যালয় ইসলামি পাঠাগার">
    <meta property="og:title" content="হোম  -  জাবিইপা" />
    <meta property="og:site_name" content="জাবিইপা" />
    <meta property="og:description" content="জাহাঙ্গীরনগর বিশ্ববিদ্যালয় ইসলামি পাঠাগার" />
    <meta property="og:url" content="https://www.juislamiclibrary.com/" />
    <meta property="og:image" content="" />

    <script type="text/javascript">
        function disableselect(e) {
            return false
        }

        function reEnable() {
            return true
        }

        //if IE4+  
        document.onselectstart = new Function("return false")
        document.oncontextmenu = new Function("return false")
        //if NS6  
        if (window.sidebar) {
            document.onmousedown = disableselect
            document.onclick = reEnable
        }
    </script>
</head>

<body>

 <!-- header section starts  -->

<header class="header">
    <div class="header-1">
        <div class="logo">
            <a href="https://juislamiclibrary.com/">
                <img src="image/juil-logo.png" alt="logo">
            </a>
        </div>

        <form action="" class="search-form">
            <input type="search" name="" placeholder="আপনার পছন্দের বই খুঁজুন..." id="search-box">
            <label for="search-box" class="fas fa-search"></label>
        </form>

        <div class="icons">
            <div id="search-btn" class="fas fa-search"></div>
            <a href="https://www.facebook.com/groups/395317002159903/" target="_blank"><i class="fab fa-facebook"></i></a>
        </div>
    </div>

    
    <div class="header-2">

        <nav class="navbar">
            <a href="index.php">হোম</a>
            <a href="index.php#featured">বইসমূহ</a>
            <a href="index.php#arrivals">নতুন বই</a>
            <a href="index.php#reviews">আমাদের সম্পর্কে</a>
            <a href="answer.php">প্রশ্ন ও উত্তর</a>
            <a href="registration.php">নিবন্ধন</a>
            <a href="contact.php">যোগাযোগ</a>
            <a href="login.php">লগইন</a>
        </nav>
    </div>

</header>


<!-- bottom navbar starts -->

<nav class="bottom-navbar">
    <a href="index.php" class="fas fa-home"></a>
    <a href="male-booklist.php" class="fa-regular fa-address-book"></a>
    <a href="female-booklist.php" class="fa-solid fa-book-open-reader"></a>
    <a href="answer.php" class="fa-sharp fa-solid fa-question"></a>
    <a href="contact.php" class="fas fa-envelope"></a>
    <a href="registration.php" class="fa-solid fa-user-plus"></a>
</nav>

<!-- bottom navbar  ends-->
<!-- header section ends -->


    <div class="fbooklist">
        <div class="heading">
            <h3><b>জাহাঙ্গীরনগর বিশ্ববিদ্যালয় ইসলামি পাঠাগার</b></h3>
            <h4>সাভার, ঢাকা-১৩৪২</h4>
        </div>
    </div>

    <section class="fbooklist">

        <h3 class="booklist">Female Section Booklist</h3>

        <table class="table table-bordered">
            <thead>
                <tr class="info">
                    <th>বই নং</th>
                    <th>বইয়ের নাম</th>
                    <th>লেখক/সম্পাদক</th>
                    <th>প্রকাশনী</th>
                    <th>পৃষ্ঠা</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <th>০১</th>
                    <td>কোরআন মাজীদ(অনুবাদ)</td>
                    <td>হাফেজ মুনীর উদ্দীন আহমেদ</td>
                    <td>আল কুরআন একাডেমী</td>
                    <td>৭৩২</td>
                </tr>
                <tr class="success">
                    <th>০২</th>
                    <td>কোরআন মাজীদ(অনুবাদ)</td>
                    <td>হাফেজ মুনীর উদ্দীন আহমেদ</td>
                    <td>আল কুরআন একাডেমী</td>
                    <td>৭৩২</td>
                </tr>
                <tr class="danger">
                    <th>০৩</th>
                    <td>কোরআন মাজীদ(অনুবাদ)</td>
                    <td>হাফেজ মুনীর উদ্দীন আহমেদ</td>
                    <td>আল কুরআন একাডেমী</td>
                    <td>৭৩২</td>
                </tr>
                <tr class="active">
                    <th>০৪</th>
                    <td>কোরআন মাজীদ(অনুবাদ)</td>
                    <td>হাফেজ মুনীর উদ্দীন আহমেদ</td>
                    <td>আল কুরআন একাডেমী</td>
                    <td>৭৩২</td>
                </tr>
                <tr class="info">
                    <th>০৫</th>
                    <td>কোরআন মাজীদ(অনুবাদ)</td>
                    <td>হাফেজ মুনীর উদ্দীন আহমেদ</td>
                    <td>আল কুরআন একাডেমী</td>
                    <td>৭৩২</td>
                </tr>
                <tr class="warning">
                    <th>০৬</th>
                    <td>সিরাতে খাতামুল আম্বিয়া</td>
                    <td>মুফতি মুহাম্মদ শফি রহ.</td>
                    <td>কালান্তর প্রকাশনী</td>
                    <td>১৪৩</td>
                </tr>
                <tr>
                    <th>০৭</th>
                    <td>সিরাতে খাতামুল আম্বিয়া</td>
                    <td>মুফতি মুহাম্মদ শফি রহ.</td>
                    <td>কালান্তর প্রকাশনী</td>
                    <td>১৪৩</td>
                </tr>
                <tr class="success">
                    <th>০৮</th>
                    <td>সিরাতে খাতামুল আম্বিয়া</td>
                    <td>মুফতি মুহাম্মদ শফি রহ.</td>
                    <td>কালান্তর প্রকাশনী</td>
                    <td>১৪৩</td>
                </tr>
                <tr class="danger">
                    <th>০৯</th>
                    <td>সিরাতে খাতামুল আম্বিয়া</td>
                    <td>মুফতি মুহাম্মদ শফি রহ.</td>
                    <td>কালান্তর প্রকাশনী</td>
                    <td>১৪৩</td>
                </tr>
                <tr class="active">
                    <th>১০</th>
                    <td>সিরাতে খাতামুল আম্বিয়া</td>
                    <td>মুফতি মুহাম্মদ শফি রহ.</td>
                    <td>কালান্তর প্রকাশনী</td>
                    <td>১৪৩</td>
                </tr>
                <tr class="info">
                    <th>১১</th>
                    <td>কুরআনের সাথে হ্রদয়ের কথা </td>
                    <td>শাইখ ইবরাহীম আস-সাকরান</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>১৬৬</td>
                </tr>
                <tr class="warning">
                    <th>১২</th>
                    <td>ইজ মিউজিক হালাল?</td>
                    <td>ড. গওহর মুশতাক</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>২২৯</td>
                </tr>
                <tr>
                    <th>১৩</th>
                    <td>জান্নাতের মনোমুগ্ধকর বর্ণনা ওপারেতে সর্বসুখ</td>
                    <td>আরিফুল ইসলাম</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>১১৭</td>
                </tr>
                <tr class="success">
                    <th>১৪</th>
                    <td>শেষরাত্রির গল্পগুলো</td>
                    <td>আব্দুল্লাহ মাহমুদ নজীব</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>১৪৯</td>
                </tr>
                <tr class="danger">
                    <th>১৫</th>
                    <td>গল্পগুলো অন্যরকম </td>
                    <td>সমকালীন টিম</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>২১১</td>
                </tr>
                <tr class="active">
                    <th>১৬</th>
                    <td>ভ্রুণের আর্তনাদ</td>
                    <td>শাহীনা বেগম</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>৯৬</td>
                </tr>
                <tr class="info">
                    <th>১৭</th>
                    <td>কাজের মাঝে রবের খোঁজে</td>
                    <td>আফিফা আনেদীন সাওদা</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>৮৩</td>
                </tr>
                <tr class="warning">
                    <th>১৮</th>
                    <td>সূরা ইউসুফ (পবিত্র এক মানবের গল্প)</td>
                    <td>শাইখ আলি জাবির আল ফাইফি</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>১২৫</td>
                </tr>
                <tr>
                    <th>১৯</th>
                    <td>ভালোবাসার রমাদান</td>
                    <td>ড আয়িয আল-কারনী</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>১৭৯</td>
                </tr>
                <tr class="success">
                    <th>২০</th>
                    <td>হিসনুল মুসলিম</td>
                    <td>ড সাইদ আল কাহতানি</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>১৭৬</td>
                </tr>
                <tr class="danger">
                    <th>২১</th>
                    <td>সেরা হোক এবারের রামাদান</td>
                    <td>রৌদ্রময়ী টীম</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>১৮০</td>
                </tr>
                <tr class="active">
                    <th>২২</th>
                    <td>জীবন যদি হতো নারী সাহাবির মতো</td>
                    <td>ড. হানান লাশিন</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>১১৯</td>
                </tr>
                <tr class="info">
                    <th>২৩</th>
                    <td>ফেরা-২</td>
                    <td>বিনতু আদিল</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>১১৯</td>
                </tr>
                <tr class="warning">
                    <th>২৪</th>
                    <td>ফেরা</td>
                    <td>সিহিন্তা শরীফা ও নাইলাহ আমাতুল্লাহ</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>১২০</td>
                </tr>
                <tr>
                    <th>২৫</th>
                    <td>প্রিয়তমা</td>
                    <td>সালাহউদ্দীন জাহাঙ্গীর</td>
                    <td>নব প্রকাশন</td>
                    <td>৩৫২</td>
                </tr>
                <tr class="success">
                    <th>২৬</th>
                    <td>হ্রদয় জাগার জন্য</td>
                    <td>ইরাসমিন মুজাহিদ</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>১৬৪</td>
                </tr>
                <tr class="danger">
                    <th>২৭</th>
                    <td>ডাবল স্টান্ডার্ড</td>
                    <td>ডা. শামসুল আরেফীন</td>
                    <td>শুদ্ধি</td>
                    <td>১৯২</td>
                </tr>
                <tr class="active">
                    <th>২৮</th>
                    <td>সিরাজাম মুনিরা</td>
                    <td>আরিফুল ইসলাম</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>১৫৬</td>
                </tr>
                <tr class="info">
                    <th>২৯</th>
                    <td>প্যারাডাক্সিকাল সাজিদ-২</td>
                    <td>আরিফ আজাদ</td>
                    <td>সমকালীন প্রকাশন</td>
                    <td>২৩৫</td>
                </tr>
                <tr class="warning">
                    <th>৩০</th>
                    <td>কল্পির কারাবাস</td>
                    <td>মুহাম্মাদ হোসাইন</td>
                    <td>শব্দতরু</td>
                    <td>৭৫</td>
                </tr>
                <tr>
                    <th>৩১</th>
                    <td>হিজাব আমার পরিচয়</td>
                    <td>জাকারিয়া মাসুদ</td>
                    <td>সন্দীপন প্রকাশন</td>
                    <td>৮৬</td>
                </tr>
                <tr class="success">
                    <th>৩২</th>
                    <td>কুরআন জীবনের গাইডলাইন</td>
                    <td>ড. ইয়াদ কুনাইবী</td>
                    <td>সন্দীপন প্রকাশন</td>
                    <td>১০৪</td>
                </tr>
                <tr class="danger">
                    <th>৩৩</th>
                    <td>খুশুঃনামাজের প্রাণ</td>
                    <td>ইমাম ইবনে রজব হাম্বলী রাহি</td>
                    <td>উমেদ প্রকাশ</td>
                    <td>১৩৬</td>
                </tr>
                <tr class="active">
                    <th>৩৪</th>
                    <td>ফুল হয়ে ফোটো</td>
                    <td>শাইখ আহমেদ মুসা জিবরীল,মোহাম্মদ হোবলস</td>
                    <td>পথিক প্রকাশন</td>
                    <td>২৪০</td>
                </tr>
                <tr class="info">
                    <th>৩৫</th>
                    <td>বিপদ যখন নিয়ামত</td>
                    <td>শাইখ মুসা জিবরীল</td>
                    <td>সমর্পন প্রকাশন</td>
                    <td>৮৬</td>
                </tr>
                <tr class="warning">
                    <th>৩৬</th>
                    <td>জীবন যেখানে যেমন</td>
                    <td>আরিফ আজাদ</td>
                    <td>সমকালীণ প্রকাশন</td>
                    <td>১৪৬</td>
                </tr>
                <tr>
                    <th>৩৭</th>
                    <td>কিয়ামুল লাইল</td>
                    <td>শাইখ আহমেদ মুসা জিবরীল</td>
                    <td>সমর্পন প্রকাশন</td>
                    <td>২৯</td>
                </tr>
                <tr class="success">
                    <th>৩৮</th>
                    <td>পারিবারিক বিপর্যয়ের কারণ</td>
                    <td>শাইখ মুহাম্মদ সালেহ আল মুনাজ্জিদ</td>
                    <td>রুহামা পাবলিকেশন</td>
                    <td>৬১</td>
                </tr>
                <tr class="danger">
                    <th>৩৯</th>
                    <td>গন্তব্য</td>
                    <td>শফিক মুন্সি</td>
                    <td>মুয়াম্মদ পাবলিকেশন</td>
                    <td>৮৯</td>
                </tr>
                <tr class="active">
                    <th>৪০</th>
                    <td>জীবন উপভোগ করুন (Enjoy Your LIfe)</td>
                    <td>ড. মুহাম্মদ ইবনে আব্দুর রহমান আরিফী</td>
                    <td>হুদহুদ প্রকাশন</td>
                    <td>৬০০</td>
                </tr>
            </tbody>
        </table>


    </section>

    <!-- footer section starts  -->
    <?php
    include "footer.php";
    ?>
    <!-- footer section ends  -->

</body>

</html>